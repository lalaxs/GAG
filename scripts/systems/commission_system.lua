...
function CommissionSystem.CompleteCommission(commission, item)
    if not CanMutateLocalState("CompleteCommission") then return false, "委托由服务器结算" end
...
function CommissionSystem.Update(dt)
    ...
    state_.timer = math.max(0, state_.timer - dt)
    local timerSecond = math.floor(state_.timer)
    if timerSecond ~= lastTimerSecond_ then
        lastTimerSecond_ = timerSecond
        EmitCommissionChanged("timer")
    end
...
function CommissionSystem.RefreshNow()
    if not CanMutateLocalState("RefreshNow") then return false end
    RefreshCommissions()
end

function CommissionSystem.HandleOffline()
    if not CanMutateLocalState("HandleOffline") then return end
...
function CommissionSystem.LoadSaveData(data)
    ...
    serverRefreshRequested_ = false
    serverRefreshRetryTimer_ = 0
